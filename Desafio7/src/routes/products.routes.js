...
productRouter.get("/:pid", async (request, response) => {
  ...
  try {
    ...
    productsById
      ? response.status(200).send({ res: "OK", message: productsById })
      : response
          .status(404)
          .send({ res: "Error", message: "Producto no encontrado" });
  } catch (error) {
    response.status(404).send({ res: "ERROR", mes: error });
  }
});
...
productRouter.put(
  ...
  async (request, response) => {
    ...
    try {
      const productByIdUpdate = await productModel.findByIdAndUpdate(pid, {
        title,
        description,
        price,
        thumbnail,
      ...
      productByIdUpdate
        ? response
            .status(200)
            .send({ res: "OK", message: "Producto actualizado" })
        : response.status(404).send({
            res: "Error actualizando el producto",
...
productRouter.delete(
  ...
  async (request, response) => {
    ...
    try {
      ...
      deleteProduct
        ? response.status(200).send({ res: "OK", message: "Producto borrado" })
        : response.status(404).send({
            res: "Error borrando el producto",
            message: "Product not found",
          });
    } catch (error) {
      response.status(404).send({ res: "ERROR", mes: error });
